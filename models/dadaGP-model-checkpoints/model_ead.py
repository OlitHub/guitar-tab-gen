...
class TransformerXL(object):
    ...
    def get_model(self, pretrain_model=None):
        ...
        st_eopch = 0
        if pretrain_model:
            checkpoint = torch.load(pretrain_model, map_location='cpu')  #  'cuda:0'
    ...
    def inference(self, model_path, token_lim, strategies, params, id, bpm, output_path, primer=1):
        ...
        '''
        beg_list = ['artist:', 'unknown_artist', 'downtune:', '0', 'tempo:','110', 'start', 'new_measure',
            'distorted0:', 'note:', 's6:', 'f0',
            'bass:','note:','s5:','f0',
            'drums:','note:','36',
            'drums:','note:','42']
        ...
        #words[-1].append(self.event2word['new_measure'])       # previously "Bar_None"
        final = "\n".join(beg_list) 
        final+='\n'
        # initialize mem
        mems = tuple()
        ...
        while len(words[0]) < token_lim:
            ...
            # between every new_measure (or after new_measure token)
            # counter for wait tokens
            # check if counter is 3840 
            # if true
            # new_measure 
        ...
        #events = [self.word2event[words[i]] for i in range(len(words))]
        ...
        #generated_file_name = "gentokens_" + "t_" + str(temperatura) + "_k_"+ str(parametro_top) +".txt"
