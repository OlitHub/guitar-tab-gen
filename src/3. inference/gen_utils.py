...
def generate_bass_ev_trans_tf(trans_bass, TransEncoders, temperature, Encoder_RG, dec_seq_length):
    ...
    RG_sos_idx = int(np.where(TransEncoders[0].categories_[0] == 'sos')[0])+1
    RG_eos_idx = int(np.where(TransEncoders[0].categories_[0] == 'eos')[0])+1 
    
    Bass_sos_idx = int(np.where(TransEncoders[1].categories_[0] == 'sos')[0])+1
    Bass_eos_idx = int(np.where(TransEncoders[1].categories_[0] == 'eos')[0])+1 
    
    dec_onset_out = []
    dec_out_onset = [RG_sos_idx]
...
def bass_trans_ev_model_tf(TransEncoders):
