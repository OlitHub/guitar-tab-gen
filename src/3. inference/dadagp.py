...
# guitar pro --> tokens
def dadagp_encode(input_file, output_file, artist_token, verbose=False):
    song = gp.parse(input_file)
    # Convert the song to tokens
    tokens = guitarpro2tokens(song, artist_token, verbose=verbose)
...
# tokens --> guitarpro
def dadagp_decode(input_file, output_file, verbose=False):
    text_file = open(input_file, "r")
    tokens = text_file.read().split("\n")

    # Convert the tokens to a song 
    song = tokens2guitarpro(tokens, verbose=verbose)
...
def dadagp_conversion(encode_or_decode, input_file, output_file, artist_token="unknown", verbose=False):
    """
    Runs the DadaGP encoder or decoder.

    Parameters:
    - encode_or_decode (str): "encode" or "decode"
    - input_file (str): Path to the input file
    - output_file (str): Path to the output file
    - artist_token (str, optional): Artist name for encoding. Defaults to "unknown".
    """
    ...
    dadagp("encode", "input.gp3", "output.txt", "artist_name")
    dadagp("encode", "examples/progmetal.gp3", "progmetal.tokens.txt", "unknown")

    DECODE (tokens --> guitar pro)
    dadagp("decode", "input.txt", "output.gp5")
    dadagp("decode", "progmetal.tokens.txt", "progmetal.decoded.gp5")
    ...
    print(f"Processing: {encode_or_decode.upper()} from {input_file} to {output_file}")

    if encode_or_decode == "encode":
        dadagp_encode(input_file, output_file, artist_token, verbose=verbose)
    else:
        dadagp_decode(input_file, output_file, verbose=verbose)
