...
class WordDecoder(tf.keras.layers.Layer):
  def __init__(self, num_layers, d_model, num_heads, dff, target_vocab,
               maximum_position_encoding, mode_choice, max_rel_pos = 256, rate=0.1):
    super(WordDecoder, self).__init__()

    self.d_model = d_model #the overall after concat
    self.num_layers = num_layers

    self.embedding1 = tf.keras.layers.Embedding(target_vocab, d_model)
  ...
  def call(self, x, enc_output,
    ...
    x = self.embedding1(x)  # (batch_size, target_seq_len, d_model1)
...
class BLSTMEncoder(tf.keras.layers.Layer):
  def __init__(self, d_model, 
    ...
    self.embedding = tf.keras.layers.Embedding(input_vocab, d_model, mask_zero=True)
...
class HybridTransformer(tf.keras.Model):
  def __init__(self, num_layers, d_model_enc, d_model_dec, num_heads, dff, input_vocab, target_vocab,
               pe_target, mode_choice, max_rel_pos_tar, rate=0.1):
    super(HybridTransformer, self).__init__()

    self.encoder = BLSTMEncoder(d_model_enc, dff, input_vocab, rate)

    self.decoder = WordDecoder(num_layers, d_model_dec, num_heads, dff, 
                           target_vocab, pe_target, mode_choice, max_rel_pos_tar, rate)

    self.final_layer_tar = tf.keras.layers.Dense(target_vocab, activation='softmax') #2 outs

  def call(self, inp, tar, look_ahead_mask, dec_padding_mask, training=False):

    enc_output = self.encoder(inp, training=training)  # (batch_size, inp_seq_len, dff)
